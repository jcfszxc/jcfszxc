...
from ..config.settings import GUISettings, ImageSettings
...
class CalibrationGUI(QMainWindow):
    ...
    def _connect_signals(self) -> None:
        """连接信号和槽"""
        self.btn_load_visible.clicked.connect(lambda: self.load_image('visible'))
    ...
    def update_magnifier(self, source_label: QLabel, pos: QPoint, original_image: np.ndarray) -> None:
        ...
        if (0 <= x < display_width and 0 <= y < display_height):
            ...
            # 定义放大区域的大小
            magnifier_size = 60  # 放大区域半径
            zoom_factor = 4  # 放大倍数
            
            # 计算放大区域的边界
            start_x = max(0, original_x - magnifier_size // zoom_factor)
            start_y = max(0, original_y - magnifier_size // zoom_factor)
            end_x = min(image_width, original_x + magnifier_size // zoom_factor)
            end_y = min(image_height, original_y + magnifier_size // zoom_factor)
            ...
            font_scale = 0.5
            cv2.putText(magnified, coordinate_text, (5, 15), font, font_scale, color, thickness)
